            ...
            <div className='flex sm:justify-center gap-4 pt-5 w-full overflow-scroll'>
                {
                    specialityData.map((data, index) => (
                        <Link onClick={() => scrollTo(0,0)} key={index} to={`/doctors/${data.speciality}`} className='flex flex-col items-center text-xs cursor-pointer shrink-0 hover:translate-y-[-10px] transition-all duration-500' >
